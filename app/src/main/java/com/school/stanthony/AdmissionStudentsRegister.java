...
public class AdmissionStudentsRegister extends AppCompatActivity {
    ...
    Runnable myRunnable = new Runnable() {
        ...
        public void run() {
            ...
            AdmissionStudentsRegister mydata = new AdmissionStudentsRegister();
            ...
            adapter = new SimpleAdapter(AdmissionStudentsRegister.this, MyData, R.layout.studentregister1, fromwhere, viewswhere);
        ...
        try
        {
            ...
            else
            {
                String query = "select distinct Receipt_No 'Receipt_No',ROW_NUMBER() OVER (ORDER BY Admission_Date)  AS Row,acadmicyear 'Year',isnull(Roll_Number,0) Roll,upper(Applicant_type) as ID,a.admission_type,(net_fee+ISNULL(charityfee,0)) 'Total',isnull(a.sportsfee,0) Sports,\n" +
                        ...
                        "      and Acadmic_Year=(select selectedaca from tblusermaster where username='"+Form1+"')) and a.class_name='"+std+"' and lower(a.division)='"+div+"'\n" +
                        "       and a.Acadmic_year=(select selectedaca from tblusermaster where username='"+Form1+"') order by Receipt_No asc";
        ...
        try
        {
            ...
            else
            {
                String query = "select distinct Receipt_No 'Receipt_No',ROW_NUMBER() OVER (ORDER BY Admission_Date)  AS Row,acadmicyear 'Year',isnull(Roll_Number,0) Roll,upper(Applicant_type) as ID,a.admission_type,(net_fee+ISNULL(charityfee,0)) 'Total',isnull(a.sportsfee,0) Sports,\n" +
                        ...
                        "      and DATEDIFF(d,convert(varchar,a.Admission_Date,101),'"+end+"')>=0 and convert(varchar,a.batch_code) in (select BatchCode from tblbatchmaster where rtrim(ltrim(lower(BatchCode)))='"+section+"'\n" +
                        "      and Acadmic_Year=(select selectedaca from tblusermaster where username='"+Form1+"')) and a.class_name='"+std+"' and lower(a.division)='"+div+"'\n" +
                        "       and a.Acadmic_year=(select selectedaca from tblusermaster where username='"+Form1+"') and a.Applicant_type='"+code+"' order by Receipt_No asc";
                Statement statement=conn.createStatement();
                ResultSet rs=statement.executeQuery(query);
