...
public class TeacherAccouncement extends AppCompatActivity {
    ...
    Runnable myRunnable = new Runnable() {
        ...
        public void run() {
            ...
            TeacherAccouncement mydata = new TeacherAccouncement();
            MyData = mydata.replacetoast(Form1);
            String[] fromwhere = {"CreatedOn","subject","msg"};
            int[] viewswhere = {R.id.Date,R.id.subject,R.id.notice};
            adapter = new SimpleAdapter(TeacherAccouncement.this, MyData, R.layout.teacherannouncement,fromwhere,viewswhere);

            if(adapter.getCount()==0){
                AlertDialog.Builder builder1 = new AlertDialog.Builder(TeacherAccouncement.this);
        ...
        try
        {
            ...
            else
            {
                String query = "select Convert(varchar(20),CreatedOn,103)CreatedOn,ForDepartment,Subject,Messg from tblTeacherNotification\n" +
                        "where ForStaff='YYY' and ForDepartment=(SELECT postnew from tbl_HRStaffnew where StaffUser='"+Form1+"' ) or  ForDepartment='yyy' and \n" +
                        "IsActive=1 order by CreatedOn desc";
                Statement statement=conn.createStatement();
                ResultSet rs=statement.executeQuery(query);

                while (rs.next()){
                    Map<String,String> datanum=new HashMap<String,String>();
                    datanum.put("CreatedOn",rs.getString("CreatedOn"));
